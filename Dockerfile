...
CMD ["bash"]
# CMD ["./bin/spark-class org.apache.spark.deploy.master.Master", "--ip localhost", "--port $SPARK_MASTER_PORT", "--webui-port $SPARK_MASTER_WEBUI_PORT"]
# ./bin/spark-class org.apache.spark.deploy.master.Master --ip localhost --port $SPARK_MASTER_PORT --webui-port $SPARK_MASTER_WEBUI_PORT
# CMD ["cat", "./bin/spark-class"]
